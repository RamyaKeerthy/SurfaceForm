...
class LSAT_Z3_Program:
    ...
    def parse_logic_program(self):
        ...
        lines = [x.strip() for x in self.logic_program.splitlines() if x.strip()]

        declaration_start_index = lines.index("# Declarations")
        constraint_start_index = lines.index("# Constraints")
        option_start_index = lines.index("# Options")

        declaration_statements = lines[declaration_start_index + 1:constraint_start_index]
        constraint_statements = lines[constraint_start_index + 1:option_start_index]
        option_statements = lines[option_start_index + 1:]

        try:
            (self.declared_enum_sorts, self.declared_int_sorts, self.declared_lists, self.declared_functions,
             self.variable_constraints) = self.parse_declaration_statements(declaration_statements)

            self.constraints = [x.split(':::')[1].strip() for x in constraint_statements]
            self.options = [x.split(':::')[1].strip() for x in option_statements if not x.startswith("Question :::")]
        except Exception as e:
            return False
    ...
    def parse_declaration_statements(self, declaration_statements):
        ...
        pure_declaration_statements = [x for x in declaration_statements if
                                       "Sort" in x or "Function" in x or "Predicate" in x]
        variable_constrant_statements = [x for x in declaration_statements if
                                         not "Sort" in x and not "Function" in x and not "Predicate" in x]
        for s in pure_declaration_statements:
            if "EnumSort" in s:
                ...
                predicate_name = s.split("=")[0].strip()
                predicate_args_str = s.split("=")[1].strip()[len("Predicate("):-1]
                predicate_args_str = predicate_args_str.replace("[", "").replace("]", "")
                predicate_args = [x.strip() for x in predicate_args_str.split(",") if x.strip()]
                function_declarations[predicate_name] = predicate_args + ["bool"]
            else:
    ...
    def execute_program(self):
        ...
        try:
            output = check_output([sys.executable, filename], stderr=subprocess.STDOUT, timeout=1.0)
